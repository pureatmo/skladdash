...
COLUMN_TOOLTIPS = {
    ...
    "Прогноз/день": "Дневной спрос, который используется для закупа. По умолчанию равен продажам/день из текущего файла; при весе истории выше 0 смешивается с историей.",
...
def _calculation_context(config: AnalysisConfig, auto_sales_period: bool) -> None:
    ...
    if history_pct:
        history_rule = (
            ...
            "Если у SKU нет продаж в новом файле, но есть история, берется исторический спрос."
        )
    else:
        history_rule = (
            "История не смешивается с текущими продажами. Если у SKU есть продажи в новом файле, "
            "прогноз считается по ним; история может подставиться только для SKU без текущих продаж."
        )
...
def _sidebar() -> tuple[AnalysisConfig, str, str, bool, bool, bool]:
    ...
    history_weight = st.sidebar.slider(
        ...
        help="0% значит: если в новом файле есть продажи, прогноз считается по нему. История используется для SKU без текущих продаж.",
    ) / 100
