...
def _file_inputs(use_local_files: bool) -> dict[str, object]:
    ...
    st.caption("Загрузите 4 Excel-файла. После загрузки расчет обновится автоматически.")
    cols = st.columns(4)
    with cols[0]:
...
def _a_stock_availability_panel(result: pd.DataFrame) -> None:
    ...
    with st.container(border=True):
        ...
        top_cols[0].metric("Наличие A по упаковкам", f"{availability_pct:.1f}%")
        top_cols[1].metric("A-упаковок в наличии", f"{available_packs:,}".replace(",", " "))
        top_cols[2].metric("A-упаковок всего", f"{total_packs:,}".replace(",", " "))
        st.caption(
        ...
        st.dataframe(critical_preview, width="stretch", height=260)
