...
DEFAULT_WEBHOOK_URLS = {
    "usa_sales": os.getenv("USA_SALES_WEBHOOK_URL", "https://n8n.1703.team/webhook/mcp-paid-orders-report-usa"),
    "ww_sales": os.getenv("WW_SALES_WEBHOOK_URL", "https://n8n.1703.team/webhook/mcp-paid-orders-report-ww"),
    "usa_stock": os.getenv("USA_STOCK_WEBHOOK_URL", "https://n8n.1703.team/webhook/us-leftovers"),
    "ww_stock": os.getenv("WW_STOCK_WEBHOOK_URL", "https://n8n.1703.team/webhook/ww-leftovers"),
}
CATEGORY_UP_STYLE = "background-color: #dcfce7; color: #166534; font-weight: 700;"
CATEGORY_DOWN_STYLE = "background-color: #fee2e2; color: #991b1b; font-weight: 700;"
STYLER_MAX_CELLS = 240_000
...
DISPLAY_COLUMNS = {
    ...
    "pack": "Упаковка",
    "abc_category": "ABCD",
    "history_last_category": "Пред. ABCD",
    "category_change_label": "Изменение ABCD",
    "stock_status": "Статус",
...
COLUMN_TOOLTIPS = {
    ...
    "Поставщик": "Seedbank из данных продаж или остатков.",
    "Товар": "Название товара из данных продаж или остатков.",
    ...
    "Пред. ABCD": "Категория этой SKU в предыдущем сохраненном отчете. Если отчеты еще не сохранялись, поле пустое.",
    "Изменение ABCD": "Показывает только изменение относительно предыдущего сохраненного отчета: вверх подсвечивается зеленым, вниз - красным.",
    ...
    "Продажи, шт.": "Количество, проданное по данным webhook за выбранный период продаж.",
    "Остаток, шт.": "Текущий свободный остаток из webhook остатков для этой упаковки.",
    "Продажи/день": "Продажи из webhook, разделенные на количество дней в выбранном периоде продаж.",
    ...
    "Коэф. сезонности": "Средний встроенный коэффициент сезонности по выбранным месяцам. 1.20 значит прогноз и закуп увеличены на 20%; 0.80 значит снижены на 20%.",
    ...
    f"Выручка, {DISPLAY_CURRENCY}": f"Выручка из webhook продаж за выбранный период. Валюта: {DISPLAY_CURRENCY}.",
    f"Маржа, {DISPLAY_CURRENCY}": f"Маржа из webhook продаж за выбранный период. Валюта: {DISPLAY_CURRENCY}.",
...
def main() -> None:
    ...
    config, run_name, notes, save_to_history, auto_sales_period = _sidebar()
    input_sources = _webhook_sources()
    seasonality_coefficients = _apply_seasonality_config(config)
    ...
    required_sources = ["usa_sales", "ww_sales", "usa_stock", "ww_stock"]
        ...
        try:
            with st.spinner("Расчет..."):
                sales, stock = load_current_data(
                    ...
                    usa_sales_webhook_url=input_sources["usa_sales"],
                    ww_sales_webhook_url=input_sources["ww_sales"],
                    usa_stock_webhook_url=input_sources["usa_stock"],
                    ww_stock_webhook_url=input_sources["ww_stock"],
                )
            ...
            st.error(f"Не удалось получить данные из webhook или выполнить расчет: {exc}")
            st.stop()
    ...
    with st.expander("Как пользоваться дашбордом", expanded=True):
        st.markdown(
            ...
            **Быстрый сценарий**
            ...
            2. Данные загружаются автоматически из n8n webhook: продажи USA, продажи WW, остатки USA и остатки WW. Excel-файлы вручную загружать не нужно.
            3. Если нужно учесть сезон, включите `Учитывать сезонность` и отметьте месяцы сезона. Коэффициенты сезонности уже зашиты в дашборд.
            ...
            **Как читать результат**
            ...
            - `Пред. ABCD` показывает категорию SKU из предыдущего сохраненного отчета. Если категория стала выше, строка подсвечивается зеленым; если ниже - красным.
            ...
            **Формула заказа**
            ...
            Webhook продаж должен возвращать строки с датами продаж; если n8n вернет уже агрегированные продажи без дат, фильтр периода применить будет нельзя.
...
def _apply_seasonality_config(config: AnalysisConfig) -> pd.DataFrame:
    config.seasonality_factors = {}
    coefficients = default_seasonality_coefficients()
...
def _seasonality_preview(coefficients: pd.DataFrame | None, config: AnalysisConfig) -> None:
    ...
    with st.expander("Коэффициенты сезонности", expanded=False):
        st.caption(
            "Коэффициенты сезонности зашиты в дашборд из блока `Продажи товаров`. "
...
def _calculation_context(config: AnalysisConfig, auto_sales_period: bool) -> None:
    ...
    if history_pct:
        ...
        history_rule = (
            "История не смешивается с текущими продажами. Если у SKU есть продажи в новых данных, "
    ...
    st.info(
        ...
        f"{seasonality_rule}"
        "Webhook продаж должен возвращать строки с датами продаж; если n8n вернет уже агрегированные продажи без дат, "
        "фильтр периода применить будет нельзя. "
        f"{history_rule}"
    )
...
def _sidebar() -> tuple[AnalysisConfig, str, str, bool, bool]:
    ...
    if auto_sales_period:
        ...
        period_start = st.sidebar.date_input(
            ...
            help="Дата начала периода продаж, который нужно взять из webhook.",
        )
    ...
    use_seasonality = st.sidebar.checkbox(
        ...
        value=False,
        help="Если включено, прогноз/день умножается на средний встроенный коэффициент выбранных месяцев.",
    )
    ...
    history_weight = st.sidebar.slider(
        ...
        help="0% значит: если в новых данных есть продажи, прогноз считается только по ним. На SKU с нулевыми продажами влияет только при включенном чекбоксе ниже.",
    ) / 100
    ...
    use_history_for_zero_sales = st.sidebar.checkbox(
        ...
        help="Если выключено, SKU с продажами 0 в текущих данных получит прогноз 0 и не попадет в закуп только из-за истории. Включайте для новых/возвращаемых SKU, где текущий период продаж не показателен.",
    )
    ...
    st.sidebar.caption("Данные берутся напрямую из n8n webhook. Файлы загружать не нужно.")
    st.sidebar.caption("Продажи USA, продажи WW, остатки USA и остатки WW обновляются при каждом перерасчете.")
...
def _webhook_sources() -> dict[str, str]:
    st.subheader("Webhook-источники")
    st.caption(
        "Дашборд сам загружает продажи и остатки из n8n при каждом изменении периода, горизонта закупа, сезонности или других параметров. "
        "Файлы Excel вручную загружать не нужно."
    )
    source_labels = {
        "usa_sales": "Заказы US",
        "ww_sales": "Заказы WW",
        "usa_stock": "Остатки US",
        "ww_stock": "Остатки WW",
    }
    with st.expander("Используемые webhook URL", expanded=False):
        for key, label in source_labels.items():
            st.code(f"{label}: {DEFAULT_WEBHOOK_URLS[key]}", language="text")
    return DEFAULT_WEBHOOK_URLS.copy()
...
def _metrics(summary: dict[str, float], run_uuid: str | None) -> None:
    ...
    cols[1].metric(
        ...
        help="Сумма количества продаж из webhook за выбранный период после фильтра по датам.",
    )
    cols[2].metric(
        "К заказу",
    ...
    cols[5].metric(
        ...
        help=f"Выручка из webhook продаж за выбранный период продаж. Валюта: {DISPLAY_CURRENCY}.",
    )
    cols[6].metric(
        "Срочно",
...
def _no_demand_tab(no_demand: pd.DataFrame) -> None:
    ...
    st.dataframe(
        _display_table(no_demand.sort_values(["current_stock", "market"], ascending=[False, True])),
        width="stretch",
        height=560,
        column_config=_table_column_config(),
    )
...
def _display_table(df: pd.DataFrame):
    ...
    if table.size > STYLER_MAX_CELLS:
        return table
    return _style_category_changes(table)


def _style_category_changes(table: pd.DataFrame):
    if table.empty or not {"ABCD", "Пред. ABCD"}.issubset(table.columns):
        return table

    highlighted_columns = [
        column for column in ["ABCD", "Пред. ABCD", "Изменение ABCD"] if column in table.columns
    ]

    def highlight_row(row: pd.Series) -> pd.Series:
        styles = pd.Series("", index=row.index)
        current = str(row.get("ABCD", "")).strip().upper()
        previous = str(row.get("Пред. ABCD", "")).strip().upper()
        if current in ABCD_RANK and previous in ABCD_RANK and current != previous:
            style = CATEGORY_UP_STYLE if ABCD_RANK[current] > ABCD_RANK[previous] else CATEGORY_DOWN_STYLE
            styles.loc[highlighted_columns] = style
        return styles
...
def _empty_state(runs: pd.DataFrame) -> None:
    st.info(
        "Дашборд готов получить данные из n8n webhook. Проверьте период продаж и параметры закупа: "
        "расчет запустится автоматически."
    )
