...
def _add_category_change(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    previous = df["history_last_category"].fillna("").astype(str).str.strip().str.upper()
    current = df["abc_category"].fillna("").astype(str).str.strip().str.upper()
    previous_rank = previous.map(ABCD_RANK)
    current_rank = current.map(ABCD_RANK)
    has_previous = previous_rank.notna()
    direction = np.sign(current_rank.sub(previous_rank).where(has_previous, 0).fillna(0)).astype(int)
    df["history_last_category"] = previous.where(has_previous, "")
    df["category_change_direction"] = direction
    df["category_change_label"] = np.select(
        [direction.gt(0), direction.lt(0)],
        ["Вверх", "Вниз"],
        default="",
    )
    return df
