...
def read_sales_webhook(
    ...
    if not url:
        raise ValueError("не указан webhook URL для продаж")
    ...
    try:
        ...
        raise ValueError(f"webhook вернул HTTP {exc.code}: {detail}") from exc
    except URLError as exc:
        raise ValueError(f"не удалось подключиться к webhook: {exc.reason}") from exc

    raw = _webhook_dataframe_from_response(data, content_type)
    return _prepare_sales(raw, market, period_start, period_end)
