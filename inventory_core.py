...
def _find_sales_date_column(columns: pd.Index) -> str | None:
    ...
    for candidate in SALES_DATE_COLUMNS:
...
def read_sales(
    ...
    if date_column and period_start and period_end:
        parsed_dates = _parse_sales_dates(raw[date_column]).dt.date
        raw = raw.loc[parsed_dates.ge(period_start) & parsed_dates.le(period_end)].copy()
